...
var Delta = function (schema1, schema2, options) {
    ...
    for (var t in _tables1) {
      ...
      if (tools.array.contains(_tables2, _tables1[t])) {
        ...
        for (var c1 in _columns1) {
          ...
          if (!_exists || _columns1[c1].comment != _columns2[c2].comment) {
        ...
        if (schema1.tables[_tables1[t]].comment) {
          _sqlTable.push('COMMENT ON ' + _tables1[t] + ' IS ' + schema1.tables[_tables1[t]].comment + ';')
        }
      }
    ...
    for (var i in _fkeys1) {
      ...
      if (!_exists) {
        ...
        if (_fk1.target_table != _fkeys2[j].target_table ||
          _fk1.target_columns.sort().join(',') != _fkeys2[j].target_columns.sort().join(',') ||
          _fk1.source_table != _fkeys2[j].source_table ||
          _fk1.source_columns.sort().join(',') != _fkeys2[j].source_columns.sort().join(',') ||
      ...
      for (var j in _types2) {
        ...
        if (_type1.name == _type2.name) {
          ...
          if (_type1.values != _type2.values) {
            ...
            // var _last = tools.array.last(_values2)
  ...
  /**
   * Generate sql insert script for selected tables
   * @private
   * @function
   */
  var __syncRows = function () {
    var _rowsData1 = schema1.tableData
    var _sqls = []
    // / cycle each table data of schema 1
    for (var tablename in _rowsData1) {
      // / delete all rows from the table
      _sqls.push('TRUNCATE ' + tablename)
      var _rows = _rowsData1[tablename].rows
      // / cycle all rows found for this table
      for (var i in _rows) {
        var _row = _rows[i]
        var _columnNames = []
        var _columnValues = []
        // / cycle all columns to retrieve column name and column value
        for (var j in _row) {
          _columnNames.push(j)
          _columnValues.push(_row[j])
        }
        _sqls.push('INSERT INTO ' + tablename + '(' + _columnNames.join(',') + ') VALUES (' + _columnValues.map(function (value) {
          // / if type is string, add '
  ...
  var __setSequencesValues = function () {
    ...
    for (var i in _sequences1) {
      ...
      // / check if both sequences have same next value
      if (_exists && _seq1.nextval != _seq2.nextval) {
        _sqls.push("SELECT setval('" + _seq1.name + "', " + _seq1.nextval + ', false)')
      }
    }
    // / if options mode is full, check if there are views in schema 2 not present in schema 1
    // / drop this sequences
    /* if (options.mode == 'full') {
      for (var i in _sequences2) {
        var _seq2 = _sequences2[i]
        // / check if sequence exists
        var _exists = false
        for (var j in _sequences1) {
          var _seq1 = _sequences1[j]
          // / sequence exists
          if (_seq1.name == _seq2.name) {
            _exists = true
            break
        ...
        // / if sequence is not in schema, drop it
        if (!_exists) {
          _sqls.push('DROP SEQUENCE ' + _seq2.name + ';')
        }
      }
    }*/
